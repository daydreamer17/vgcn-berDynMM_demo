...
class VGCNBertDynMM_NoDynMM(nn.Module):
    def __init__(self, bert_model_name, gcn_in_dim, gcn_hidden_dim, num_classes, char_vocab_size, char_emb_dim, layer_idx=-1):
        ...
        # Update the input dimension to 1536 since we are concatenating two 768-sized vectors
        self.fc = nn.Linear(1536, num_classes)  # Change from 768 to 1536
    ...
    def forward(self, input_ids, attention_mask, graph, char_ids):
        ...
        # Directly concatenate BERT output and GCN output
        ...
        # Concatenate BERT and GCN output
        combined_output = torch.cat([bert_output, gcn_output], dim=1)

        logits = self.fc(combined_output)  # Now this should work because the input size is 1536
        return logits
